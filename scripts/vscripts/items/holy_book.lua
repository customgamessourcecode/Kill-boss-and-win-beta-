...
local illusion_damage_decrease = 4
   				...
   				Timers:CreateTimer(fly_time, function() -- таймер для спавна нейтралов
      							if x and IsValidEntity(x) and x:IsAlive() and caster and IsValidEntity(caster) then
      								if x:GetUnitName() == "npc_dota_hero_meepo" then
      									damage = damage/illusion_damage_decrease
      								end
      								if target then
      								...
      								ApplyDamage({ victim = x, attacker = caster, damage = damage, damage_type = DAMAGE_TYPE_PHYSICAL, damage_flags = DOTA_DAMAGE_FLAG_IGNORES_PHYSICAL_ARMOR, ability = ability})
      							end
				      			return nil
  								end )
			end
