function SpendChargeRight(keys)
	local abilityS = keys.ability
	local CountShards = abilityS:GetCurrentCharges()
	if CountShards == 1
	then
	keys.caster:RemoveItem(abilityS)
	end
	if CountShards > 1
	then
	abilityS:SetCurrentCharges(abilityS:GetCurrentCharges() - 1)
	end
end
